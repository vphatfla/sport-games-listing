const getNLBData = async (req, res) => {
    const dates = req.query.dates

    var param = (dates === null || dates === '' || dates == undefined) ? '' : '?dates=' + dates

    try {
        const data = await (await fetch("https://site.api.espn.com/apis/site/v2/sports/baseball/mlb/scoreboard" + param)).json()
